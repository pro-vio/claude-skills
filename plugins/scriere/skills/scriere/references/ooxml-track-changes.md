...
### `w:lastRenderedPageBreak` — the invisible MISS

The **second** big source of unexplained "MISS". Word stamps `<w:lastRenderedPageBreak/>` as the
first child of the run where it last paginated, so an ordinary text run becomes
`w:r[lastRenderedPageBreak, w:t]`. Any `.docx` Word has opened and paginated has these
(24 of them in a 30-page manuscript), and they land wherever the page happens to break —
i.e. on text that is otherwise perfectly matchable. `_pure_text_run` used to refuse those runs,
so `replace`/`delete`/`insert_after_text` reported MISS even for a single-word anchor sitting
in a single run, with no reason given.

ECMA-376 defines the element as **informative**: Word recomputes it at the next pagination, so
it affects neither text nor layout. `_pure_text_run` therefore accepts it (`_IGNORABLE_IN_RUN`),
and the rebuilt fragments — which come from `_mk_t_like`, copying `w:rPr` only — simply **drop**
it. Dropping is the point: keeping it while splitting a run would need a rule for which half
gets it, and putting it in both would give Word two page breaks. Losing it costs nothing.

Do **not** extend `_IGNORABLE_IN_RUN` by analogy. `w:noBreakHyphen` and `w:softHyphen` look
similar but are authored *characters*; `w:tab` and `w:br` carry layout. Only
`lastRenderedPageBreak` is regenerated by Word and safe to discard. A run that pairs the break
with a `w:drawing` is still refused — the drawing is what makes it non-pure.

This is distinct from `_run_groups`' `_TRANSPARENT` set (`w:proofErr`, `w:bookmarkStart`,
`w:bookmarkEnd`), which skips *paragraph-level* children so they don't fragment a run group.
`lastRenderedPageBreak` lives *inside* a run, so it needs the separate `_pure_text_run` rule.
